...
impl HyperliquidCommission {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.maker_rate < 0.0 || self.maker_rate > 1.0 {
            return Err(HyperliquidBacktestError::validation(format!(
                "Invalid maker rate: {}. Must be between 0.0 and 1.0",
                self.maker_rate
            )));
        }
        if self.taker_rate < 0.0 || self.taker_rate > 1.0 {
            return Err(HyperliquidBacktestError::validation(format!(
                "Invalid taker rate: {}. Must be between 0.0 and 1.0",
                self.taker_rate
            )));
        }
...
impl HyperliquidBacktest {
    ...
    pub fn new(
        ...
        Self {
            ...
            order_type_strategy: OrderTypeStrategy::Mixed {
                maker_percentage: 0.5,
            },
            funding_pnl: Vec::new(),
            trading_pnl: Vec::new(),
            total_pnl: Vec::new(),
    ...
    pub fn initialize_base_backtest(&mut self) -> Result<()> {
        ...
        // Create a simple SMA crossover strategy based on strategy name
        let strategy =
            if self.strategy_name.contains("Scalping") || self.strategy_name.contains("EMA") {
                self.create_ema_crossover_strategy(rs_data.clone(), 3, 8)
            } else if self.strategy_name.contains("SMA") {
                self.create_sma_crossover_strategy(rs_data.clone(), 5, 15)
            } else {
                // Default to a simple momentum strategy
                self.create_momentum_strategy(rs_data.clone())
            };
    ...
    pub fn calculate_with_funding(&mut self) -> Result<()> {
        ...
        // Reset funding totals and commission tracking
        self.total_funding_paid = 0.0;
        self.total_funding_received = 0.0;
        self.funding_payments.clear();
        self.commission_tracker = CommissionTracker::default();

        // Get actual trade data from base backtest
        let base_backtest = self.base_backtest.as_ref().unwrap();
        let orders = base_backtest.orders();
        let positions = base_backtest.position();
        let account_values = base_backtest.account();

        let mut cumulative_funding_pnl = 0.0;
        let mut cumulative_trading_pnl = 0.0;
        ...
        for i in 0..data_len {
            ...
            // Get current position from base backtest
            let current_position = if i < positions.len() {
                positions[i]
            } else {
                0.0
            };

            // Check for trade execution and calculate commission
            if i > 0 && i < orders.len() {
                let current_order = orders[i];
                let previous_order = orders[i - 1];

                // Detect when a trade occurs (order changes and is not NULL)
                if current_order != previous_order
                    && current_order != rs_backtester::orders::Order::NULL
                {
                    trade_count += 1;

                    // Calculate trade value (position size * price)
                    let position_change = if i < positions.len() && i > 0 {
                        (positions[i] - positions[i - 1]).abs()
                    } else {
                        1.0 // Default trade size
                    };

                    let trade_value = position_change * price;

                    // Determine trading scenario
                    let scenario = if positions.get(i - 1).unwrap_or(&0.0).abs() == 0.0 {
                        TradingScenario::OpenPosition
                    } else if current_position.abs() == 0.0 {
                        TradingScenario::ClosePosition
                    } else if current_position.abs() > positions.get(i - 1).unwrap_or(&0.0).abs() {
                        TradingScenario::IncreasePosition
                    } else {
                        TradingScenario::ReducePosition
                    };

                    // Calculate commission for this trade
                    let (order_type, commission) =
                        self.calculate_trade_commission(trade_value, trade_count, scenario);

                    // Track commission
                    self.track_commission(timestamp, order_type, trade_value, commission, scenario);

                    // Subtract commission from trading PnL
                    cumulative_trading_pnl -= commission;
                }
            }

            // Calculate base trading PnL (without commission)
            if i < account_values.len() {
                let account_pnl = account_values[i] - self.initial_capital;
                cumulative_trading_pnl = account_pnl - self.commission_tracker.total_commission();
            }
            ...
            if self.is_funding_time(timestamp) {
                ...
                if let Some(funding_rate) = self.get_funding_rate_for_timestamp(timestamp) {
                    ...
                    let funding_payment =
                        self.calculate_funding_payment(current_position, funding_rate, price);
            ...
            // Store cumulative PnL values
            self.funding_pnl[i] = cumulative_funding_pnl;
            self.trading_pnl[i] = cumulative_trading_pnl;
            self.total_pnl[i] = cumulative_trading_pnl + cumulative_funding_pnl;
        }
        ...
        for i in 0..data_len {
            ...
            if self.is_funding_time(timestamp) {
                ...
                if let Some(funding_rate) = self.get_funding_rate_for_timestamp(timestamp) {
                    // Calculate funding payment
                    let funding_payment =
                        self.calculate_funding_payment(position_size, funding_rate, price);
    ...
    fn update_enhanced_metrics(&mut self) -> Result<()> {
        ...
        self.enhanced_metrics.funding_payments_received = self
            .funding_payments
            .iter()
            .filter(|p| p.payment_amount > 0.0)
            .count();
        self.enhanced_metrics.funding_payments_paid = self
            .funding_payments
            .iter()
            .filter(|p| p.payment_amount < 0.0)
            .count();
        ...
        if !self.funding_payments.is_empty() {
            let total_funding_rate: f64 =
                self.funding_payments.iter().map(|p| p.funding_rate).sum();
            self.enhanced_metrics.average_funding_rate =
                total_funding_rate / self.funding_payments.len() as f64;
            ...
            let variance: f64 = self
                ...
                .sum::<f64>()
                / self.funding_payments.len() as f64;
        ...
        if self.funding_pnl.len() > 1 {
            let returns: Vec<f64> = (1..self.funding_pnl.len())
                .map(|i| {
                    let prev_total =
                        self.initial_capital + self.trading_pnl[i - 1] + self.funding_pnl[i - 1];
                    let curr_total =
                        self.initial_capital + self.trading_pnl[i] + self.funding_pnl[i];
                    if prev_total > 0.0 {
    ...
    pub fn calculate_trade_commission(
        ...
        let commission =
            self.commission_config
                .calculate_scenario_fee(scenario, order_type, trade_value);
    ...
    /// Create an EMA crossover strategy for scalping
    fn create_ema_crossover_strategy(
        &self,
        data: rs_backtester::datas::Data,
        fast_period: usize,
        slow_period: usize,
    ) -> rs_backtester::strategies::Strategy {
        // Simple strategy that generates alternating buy/sell signals for testing
        let data_len = self.data.len(); // Use self.data instead of the private field
        let mut choices = Vec::new();

        // Create simple alternating signals for scalping (buy/sell every few periods)
        for i in 0..data_len {
            let signal = if i < slow_period {
                rs_backtester::orders::Order::NULL
            } else {
                // Simple pattern: alternate between buy and sell every 3-5 periods
                let cycle = (i / 4) % 3;
                match cycle {
                    0 => rs_backtester::orders::Order::BUY,
                    1 => rs_backtester::orders::Order::SHORTSELL,
                    _ => rs_backtester::orders::Order::NULL,
                }
            };
            choices.push(signal);
        }
    ...
    fn create_sma_crossover_strategy(
        &self,
        data: rs_backtester::datas::Data,
        fast_period: usize,
        slow_period: usize,
        ...
        // Create simple SMA-based signals
        for i in 0..data_len {
            let signal = if i < slow_period {
                rs_backtester::orders::Order::NULL
            } else {
                // Simple pattern: buy on every 5th period, sell on every 7th
                if i % 5 == 0 {
                    rs_backtester::orders::Order::BUY
                } else if i % 7 == 0 {
                    rs_backtester::orders::Order::SHORTSELL
                } else {
                    rs_backtester::orders::Order::NULL
                }
            };
    ...
    fn create_momentum_strategy(
        ...
        for i in 0..data_len {
            let signal = if i < 5 {
                ...
                // Simple momentum pattern: buy every 6th period, sell every 8th
                if i % 6 == 0 {
                    rs_backtester::orders::Order::BUY
                } else if i % 8 == 0 {
                    rs_backtester::orders::Order::SHORTSELL
    ...
    pub fn funding_summary(&self) -> FundingSummary {
        ...
        let average_funding_payment = if funding_payment_count > 0 {
            let total_payments: f64 = self.funding_payments.iter().map(|p| p.payment_amount).sum();
            total_payments / funding_payment_count as f64
        } else {
            0.0
        };

        let funding_contribution_percentage =
            if self.enhanced_metrics.total_return_with_funding != 0.0 {
                (self.enhanced_metrics.funding_only_return
                    / self.enhanced_metrics.total_return_with_funding)
                    * 100.0
            } else {
                0.0
    ...
    pub fn enhanced_report(&self) -> Result<EnhancedReport> {
        ...
        // Calculate final equity including commissions and funding
        let base_final_equity = if let Some(last_position) = base_backtest.position().last() {
        ...
        // Subtract total commissions paid and add net funding
        let final_equity = base_final_equity - self.commission_tracker.total_commission()
            + (self.total_funding_received - self.total_funding_paid);
    ...
    pub fn print_enhanced_report(&self) -> Result<()> {
        ...
        println!(
            ...
            self.data
                .datetime
                .first()
                .unwrap_or(&DateTime::parse_from_rfc3339("1970-01-01T00:00:00Z").unwrap()),
            self.data
                .datetime
                .last()
                .unwrap_or(&DateTime::parse_from_rfc3339("1970-01-01T00:00:00Z").unwrap())
        ...
        println!(
            "Total Return (with Funding): {:.2}%",
            report.enhanced_metrics.total_return_with_funding * 100.0
        );
        println!(
            "Trading-Only Return: {:.2}%",
            report.enhanced_metrics.trading_only_return * 100.0
        );
        println!(
            "Funding-Only Return: {:.2}%",
            report.enhanced_metrics.funding_only_return * 100.0
        );
        println!(
            "Sharpe Ratio (with Funding): {:.2}",
            report.enhanced_metrics.sharpe_ratio_with_funding
        );
        println!(
            "Max Drawdown (with Funding): {:.2}%",
            report.enhanced_metrics.max_drawdown_with_funding * 100.0
        );
        ...
        println!(
            "Maker Fees: ${:.2} ({} orders)",
            report.commission_stats.maker_fees, report.commission_stats.maker_orders
        );
        println!(
            "Taker Fees: ${:.2} ({} orders)",
            report.commission_stats.taker_fees, report.commission_stats.taker_orders
        );
        println!(
            "Average Commission Rate: {:.4}%",
            report.commission_stats.average_rate * 100.0
        );
        println!(
            "Maker/Taker Ratio: {:.2}",
            report.commission_stats.maker_taker_ratio
        );

        // Print funding summary
        println!("\n--- Funding Summary ---");
        println!(
            "Total Funding Paid: ${:.2}",
            report.funding_summary.total_funding_paid
        );
        println!(
            "Total Funding Received: ${:.2}",
            report.funding_summary.total_funding_received
        );
        println!("Net Funding: ${:.2}", report.funding_summary.net_funding);
        println!(
            "Funding Payments: {}",
            report.funding_summary.funding_payment_count
        );
        println!(
            "Average Funding Payment: ${:.2}",
            report.funding_summary.average_funding_payment
        );
        println!(
            "Average Funding Rate: {:.6}%",
            report.funding_summary.average_funding_rate * 100.0
        );
        println!(
            "Funding Rate Volatility: {:.6}%",
            report.funding_summary.funding_rate_volatility * 100.0
        );
        println!(
            "Funding Contribution: {:.2}% of total return",
            report.funding_summary.funding_contribution_percentage
        );
    ...
    pub fn export_to_csv<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        for i in 0..self.data.len() {
            ...
            let total_pnl = (self.trading_pnl.get(i).unwrap_or(&0.0)
                + self.funding_pnl.get(i).unwrap_or(&0.0))
            .to_string();

            let equity = (self.initial_capital
    ...
    pub fn export_report_to_csv<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        // Enhanced metrics
        wtr.write_record(&[
            "Total Return (with Funding)",
            &(report.enhanced_metrics.total_return_with_funding * 100.0).to_string(),
        ])?;
        wtr.write_record(&[
            "Trading-Only Return",
            &(report.enhanced_metrics.trading_only_return * 100.0).to_string(),
        ])?;
        wtr.write_record(&[
            "Funding-Only Return",
            &(report.enhanced_metrics.funding_only_return * 100.0).to_string(),
        ])?;
        wtr.write_record(&[
            "Sharpe Ratio (with Funding)",
            &report
                .enhanced_metrics
                .sharpe_ratio_with_funding
                .to_string(),
        ])?;
        wtr.write_record(&[
            "Max Drawdown (with Funding)",
            &(report.enhanced_metrics.max_drawdown_with_funding * 100.0).to_string(),
        ])?;

        // Commission statistics
        wtr.write_record(&[
            "Total Commission",
            &report.commission_stats.total_commission.to_string(),
        ])?;
        wtr.write_record(&[
            "Maker Fees",
            &report.commission_stats.maker_fees.to_string(),
        ])?;
        wtr.write_record(&[
            "Taker Fees",
            &report.commission_stats.taker_fees.to_string(),
        ])?;
        wtr.write_record(&[
            "Maker Orders",
            &report.commission_stats.maker_orders.to_string(),
        ])?;
        wtr.write_record(&[
            "Taker Orders",
            &report.commission_stats.taker_orders.to_string(),
        ])?;
        wtr.write_record(&[
            "Average Commission Rate",
            &(report.commission_stats.average_rate * 100.0).to_string(),
        ])?;
        wtr.write_record(&[
            "Maker/Taker Ratio",
            &report.commission_stats.maker_taker_ratio.to_string(),
        ])?;

        // Funding summary
        wtr.write_record(&[
            "Total Funding Paid",
            &report.funding_summary.total_funding_paid.to_string(),
        ])?;
        wtr.write_record(&[
            "Total Funding Received",
            &report.funding_summary.total_funding_received.to_string(),
        ])?;
        wtr.write_record(&[
            "Net Funding",
            &report.funding_summary.net_funding.to_string(),
        ])?;
        wtr.write_record(&[
            "Funding Payments",
            &report.funding_summary.funding_payment_count.to_string(),
        ])?;
        wtr.write_record(&[
            "Average Funding Payment",
            &report.funding_summary.average_funding_payment.to_string(),
        ])?;
        wtr.write_record(&[
            "Average Funding Rate",
            &(report.funding_summary.average_funding_rate * 100.0).to_string(),
        ])?;
        wtr.write_record(&[
            "Funding Rate Volatility",
            &(report.funding_summary.funding_rate_volatility * 100.0).to_string(),
        ])?;
        wtr.write_record(&[
